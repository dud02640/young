              ...
              <div class="col-md-12">
                <table class="table table-bordered dataTable" id="dataTable" role="grid" width="100%" cellspacing="0">
                	...
                	<c:forEach var="selectCheckListAll" items="${selectCheckListAll}" varStatus="status">
                		<tr>
                			<td><input type="checkbox" name="checkbox" value="${selectCheckListAll.workNo}"></td>
							<td>${status.count}</td>
							<td><a href="#" data-toggle="modal" data-target="#updateCheckListModal" onclick="updateListModalIdPage(${selectCheckListAll.workNo})" data-dismiss="modal">${selectCheckListAll.workNum}</a></td>
							<td>${selectCheckListAll.workSubject}</td>
							<td>${selectCheckListAll.startDate} ~ ${selectCheckListAll.endDate}</td>
							<c:choose>
							<c:when test="${params.adminYn=='N'}">
							</c:when>
							<c:otherwise>
